...
namespace salvador{
    ...
    std::ostream& operator<<(std::ostream &out, const Matriz &A){
        int i,j,ren = A.get_renglones(), col = A.get_columnas()
                ,precision{ 8 },espacio{precision+1+2+1} ;
    ...
    ****************************************************/
    ...
    Matriz Matriz::ingresaMatriz(){
        ...
        for(int i =0; i<ren; i++) {
            for(int j=0; j<col; j++) {
                std::cout <<"Write the value of the matrix in ["
                            << i+1 <<"]["<< j+1 <<"]: ";
                std::cin >> mat[i][j];
                std::cin.get(); //to get rid of the enter
            }
        }
        std::cout << std::endl;
    return *this;}
    ...
    //*
    Matriz Matriz::inputMatDisplay(int pos){ //0 todos, 1 solo positivos
        int temp;
        ...
        for(int i = 0; i < ren; i++){
            for (int j = 0; j < col; j++){
                ...
                std::cout << std::endl <<"Matriz Actual: ren(" << i <<"), col("
                            << j <<")" << std::endl;
                if(!(i== 0 && j== 0) ) Matriz::print_Mat(i,j-1);
                while (1) {
            ...
            switch(opt){
                ...
                case 'N':
                    ...
                    while(scanf(" %d %d %d",&aux_ren,&aux_col,&nuevo) != 3)
                            while((c = getchar()) != '\n' && c != EOF);
                    Matriz::change_val(aux_ren,aux_col,nuevo);
                    break;
